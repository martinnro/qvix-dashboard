                        ...
                        {materiales.map((r, i) => {
                          ...
                          return (
                            <Fragment key={r.material}>
                              <tr onClick={() => { if (expandedMaterial === r.material) { setExpandedMaterial(null); return; } setExpandedMaterial(r.material); if (materialDetails[r.material]) return; setLoadingDetail(r.material); const p = new URLSearchParams({ material: r.material }); if (tipo === "instalaciones") p.set("tipo", "instalaciones"); if (filters.sucursal) p.set("sucursal", filters.sucursal); if (filters.mesAnio) p.set("mes_anio", filters.mesAnio); else if (filters.anio) p.set("anio", filters.anio); fetch(`/api/materiales-historial?${p}`).then(res => res.json()).then(data => setMaterialDetails(prev => ({ ...prev, [r.material]: data.rows ?? [] }))).catch(() => {}).finally(() => setLoadingDetail(null)); }} className="border-b border-slate-800 hover:bg-slate-700/30 transition-colors cursor-pointer">
